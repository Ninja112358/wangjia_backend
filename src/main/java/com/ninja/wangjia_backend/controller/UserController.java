...
import com.ninja.wangjia_backend.model.dto.user.*;
...
public class UserController {
    ...
    public BaseResponse<Boolean> updateUser(@RequestBody UserUpdateRequest userUpdateRequest) {
        ...
        long count = userService.count(new QueryWrapper<User>().eq("userAccount", userUpdateRequest.getUserAccount()));
        //用户名如果已存在并且不是当前用户的用户名
        ThrowUtils.throwIf(count > 0 && !userUpdateRequest.getUserAccount().equals(userService.getById(userUpdateRequest.getId()).getUserAccount()), ErrorCode.OPERATION_ERROR,"用户名已存在");
        User user = new User();
